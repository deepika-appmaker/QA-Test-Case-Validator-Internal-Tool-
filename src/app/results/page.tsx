...
function ResultsContent() {
    ...
    const savePendingFile = async (rows: TestCase[], meta: any) => {
        console.log('savePendingFile called', { user: !!user, isSaving: isSaving.current, rows: rows.length });
        if (!user || isSaving.current) return;
        isSaving.current = true;

        try {
            console.log('Getting file version...', meta);
            ...
            console.log('Saving test cases...', newFileId);
            await saveTestCases(newFileId, rows);
            
            console.log('Test cases saved, setting fileId and loading history...');
            setFileId(newFileId);
    ...
    const handleAnalyze = useCallback(async () => {
        console.log('handleAnalyze Triggered', { analyzing, testCasesLength: testCases.length });
        if (analyzing) return;
        setAnalyzing(true);
        setAnalyzedCount(0);
    ...
    useEffect(() => {
        ...
        console.log('Auto-Run Effect Evaluated', { shouldAutoRun, fileId, testCasesLength: testCases.length, analyzing, analyzed });
